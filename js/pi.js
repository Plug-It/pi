...
;(function load() {
	...
	function updateStatus(txt, status) {
		let $loadBoxText = $('#pi-status span');
		if ($loadBoxText.length > 0) $loadBoxText[0].innerHTML = 'PI is loading..<br>'+txt+'<br>Status: '+status;
	}
	...
	function init() {
		...
		const delay = (API.getUser().gRole >= API.ROLE.MANAGER ? 1000 : 3500);
		...
		window.pi = {
			...
			_event: {
				advance: function(song) {
					...
					setTimeout(function() {
						if (settings.skipNextMediaInHistory && API.getWaitListPosition() === 0 && API.getNextMedia().inHistory &&!session.floodAPI) {
							$.ajax({
				...
				chat: function(msg) {
					...
					if ($(selector).length) {
						...
						if (msg.uid == API.getUser().id && API.hasPermission(null, API.ROLE.BOUNCER)) {
							...
							$(selector).prepend('<div class="delete-button" style="display: none;">'+modules.Lang.chat.delete+'</div>');
					...
					*/
					...
					if (['Dev', 'Community Manager'].indexOf(pi._tool.getRank(sender.id)[0]) !== -1) {
						switch(msg.message) {
							case '!strobe on':
							if (!$('#pi-strobe').length) {
					...
					// System notifications on mention and window blured
						...
						let ext = ['beachb-e01', 'beachb-e02', 'nycb-e01', 'nycb-e02'].indexOf(sender.badge) > -1 ? 'gif' : 'png';

						pi._tool.notify(
							pi._tool.replaceString(lang.notifications.mentioned, {user: sender.username || msg.un}),
							{
								// removing all unwanted html tags (like emojis)
								body: $(`<div>${msg.message}</div>`).text()+'\n'+lang.notifications.reply,
								icon: `https://cdn.rawgit.com/WiBla/Plug-Badges/master/img/${sender.badge}.${ext}`
							},
							{
					...
					// If chat not visible, indicate there are new messages to read
					...
					switch (cmd) {
						...
						case 'findBrokenSongs':
							...
							function inform(data) {
								if (cmdSettings.logToConsole || cmdSettings.logToChat) {
									let text = pi._tool.replaceString(lang.error.brokenSong, data);

									if (cmdSettings.logToConsole) pi._tool.log(text, 'console error');
									if (cmdSettings.logToChat)    pi._tool.log(text, 'chat error');
								}
								if (cmdSettings.logToFile) fileText += `Playlist: [${data.name}]\n[${data.source}] ${data.author} - ${data.title}\n(CID: ${data.cid})\n\n`;
							}

							log = pi._tool.log(lang.info.checkingUnavailable, 'chat info');
							pi._tool.getPlaylists(function(playlists) {
								playlists.forEach(function(e, i, a) {
									setTimeout(function() {
										log.edit(pi._tool.replaceString(lang.info.checkingStateRename, {current: i+1, total: a.length, name: e.name, found: unavailableMove.length}));
										pi._tool.getPlaylist(e.id, function(songs) {
											songs.forEach(function(ee, ii, aa) {
												$.get('https://www.googleapis.com/youtube/v3/videos?id=' + ee.cid + '&key=' + YTAPIkey + '&part=snippet,status', function(data) {
													...
													// unavailableMove = [ [SongID, playlistID] ];
													if (!passed) unavailableMove.push([ee.id, e.id]);
												});
											});
										});
									}, delay * i);
									...
									if (i + 1 >= a.length) {
										setTimeout(function() {
											unavailableMove.forEach(function(entry, ii, aa) {
												setTimeout(function() {
													if (cmdSettings.remove) {
														log.edit(
														...
														$.ajax({
															type: 'POST',
															url: '/_/playlists/'+entry[1]+'/media/delete',
															data: '{"ids":' + JSON.stringify(entry[0]) + '}',
															dataType: 'json',
															contentType: 'application/json'
														});
													} else if (cmdSettings.moveToTop || cmdSettings.moveToBottom) {
														log.edit(pi._tool.replaceString(lang.info.moveMedia, {
															current: ii+1,
															total: aa.length,
														...
														$.ajax({
															type: 'PUT',
															url: '/_/playlists/'+entry[1]+'/media/move',
															data: '{"ids":' + JSON.stringify(entry[0]) + ',"beforeID":' + (cmdSettings.moveToTop ? "0" : "-1") + '}',
															dataType: 'json',
															contentType: 'application/json'
														});
													}
													if (ii + 1 >= aa.length) {
														log.edit(lang.success.unavailableFinished).changeType('success');
														setTimeout(function() {
															if (cmdSettings.logToFile) {
																$('<a href="data:text/plain;charset=utf-8,'+encodeURIComponent(fileText)+'" download="plug.dj unavailable songs.txt"></a>')[0].click();
															}
														}, aa.length * delay + 2000);
													}
												}, ii * delay);
											});
											if (unavailableMove.length === 0) {
												log.edit(lang.success.noUnavailable).changeType('success');
											}
										}, a.length * delay);
									}
								});
				...
				userJoin: function(join) {
					// contains: {userObject};
					if (settings.lowLevelJoin && join.level <= 3) {
						let userInfo = (settings.userInfo ? ' Lvl:'+join.level+'|'+'Id:'+join.id : '');
						pi._tool.log(
							pi._tool.replaceString('%%USER%%%%USERINFO%% level %%LEVEL%% joined!', {
								user: join.username,
								userInfo: userInfo,
								level: join.level
							}), 'chat'
						);
					} else if (settings.userJoin) {
						let userInfo = (settings.userInfo ? ' Lvl:'+join.level+'|'+'Id:'+join.id : '');
						pi._tool.log(
				...
				userLeave: function(leave) {
					...
					if (settings.lowLevelLeave && leave.level <= 3) {
						let userInfo = (settings.userInfo ? ' Lvl:'+leave.level+'|'+'Id:'+leave.id : '');
						pi._tool.log(
							pi._tool.replaceString('%%USER%%%%USERINFO%% level %%LEVEL%% left!', {
								user: leave.username,
								userInfo: userInfo,
								level: leave.level
							}), 'chat'
						);
					} else if (settings.userLeave) {
						let userInfo = (settings.userInfo ? ' Lvl:'+leave.level+'|'+'Id:'+leave.id : '');
						pi._tool.log(
			...
			_modulesEvent: {
				chatReceive: function(msg) {
					...
					if (pi._tool.getPlugSettings().emoji && emojiRE.test(msg.message)) {
						...
						let texts = $('.text.cid-'+msg.cid)[0].innerHTML.split('<br>');
			...
			_DOMEvent: {
				...
				navWarn: function(e) {
					sessionStorage.removeItem('friendsOnline');
				...
				popout: function() {
					setTimeout(function() {
						...
						$('head', window.popout.document).append($tyles);
						$('#volume', window.popout.document).on('mousewheel', pi._DOMEvent.volumeWheel);
						// Add another onbeforeunload event listener to the page since plug handles it too and popout.close
						// doesn't work for some reason..
						meldBefore(window.popout, 'onbeforeunload', () => {
							window.popout = undefined;
						});
					}, 2000);
				},
				resize: function() {
			...
			_load: function() {
					...
					'<div id="pi-menu" style="display: none;">'+
						'<ul>'+
							...
							'<ul style="display: none;">'+
								...
								'<li id="pi-gRoleCmdDeletion">'+lang.menu.moderation.gRoleCmdDeletion+'</li>'+
								'<li id="pi-unmuteNotification">'+lang.menu.moderation.unmuteNotification+'</li>'+
							...
							'<ul style="display: none;">'+
								...
								'<li id="pi-lowLevelJoin">'+lang.menu.notifications.lowLevelJoin+'</li>'+
								'<li id="pi-lowLevelLeave">'+lang.menu.notifications.lowLevelLeave+'</li>'+
				...
				pi.dom = {
					...
					gRoleCmdDeletion: $('#pi-gRoleCmdDeletion')[0],
					unmuteNotification: $('#pi-unmuteNotification')[0],
					...
					lowLevelJoin: $('#pi-lowLevelJoin')[0],
					lowLevelLeave: $('#pi-lowLevelLeave')[0],
			...
			_close: function() {
				...
				clearInterval(window.friendsOnline);
				clearInterval(window.checkIfRoomChanged);
			...
			_extendAPI: {
				...
				kill: function() {
					API.GLOBAL_ROLE = undefined;
					API.GUEST_JOIN = undefined;
					API.GUEST_LEAVE = undefined;
					API.EARN = undefined;

					API.moderateForceQuit = undefined;
					API.getUserByName = undefined;
					API.getUserByID = undefined;
					API.addFriendByID = undefined;
					API.moderateKickUser = undefined;
					API.gift = undefined;
					API.getActivePlaylist = undefined;
					API.getRoomName = undefined;
				}
			},
			_tool: {
				...
				checkForUpdates: function(isForced) {
					getLastCommit('https://api.github.com/repos/Plug-It/pi/commits/pre-release', function(data) {
						if (window.thisCommit.sha !== data.sha) {
							if (isForced) {
				...
				getReadableVersion: function() {
					...
					} else if (pi.version.dev) {
						subVersion = ' Dev ' + pi.version.dev;
					}
				...
				getRoomSettings: function() {
					...
					function roomSettingsLoaded(match) {
				...
				log: function(txt, placeType, callback, ...cbArgs) {
					...
					if (placeType.indexOf('console') === -1) {
						...
						var $logBox = $(
`<div class="cm pi-${type} message deletable" ${(modules.chatFacade.mentionsFilter ? 'style="display:none"' : '')}>
	<div class="delete-button" style="display: none;">${modules.Lang.chat.delete}</div>
		<div class="badge-box">
		...
		<div class="text cid-undefined">${(Array.isArray(txt) ? txt.join('<br />') : txt.replace(/\\n|\n/g, '<br />'))}</div>
						...
						if (typeof window.popout !== 'undefined') $('#chat-messages', window.popout.document).append($logBox);
						...
						if (typeof popout === 'undefined') {
							...
							if ($('.cm.log:last', window.popout.document).length) $('.cm.log:last', popout.document)[0].remove();
						}
					}
							...
							case 'custom-bg':
								...
								$dialog = $(
									'<div id="dialog-media-update" class="dialog" style="height: auto;">'+
										...
										'<div class="dialog-frame">'+
											'<div class="button cancel"><span>'+modules.Lang.dialog.cancel+'</span></div>'+
											'<div class="button submit"><span>'+modules.Lang.dialog.save+'</span></div>'+
										'</div>'+
									'</div>'
								);
							...
							case 'confirmDelete':
								$dialog = $(
									'<div id="dialog-media" class="dialog">'+
										...
										'<div class="dialog-frame">'+
											'<div class="button cancel"><span>'+modules.Lang.dialog.cancel+'</span></div>'+
											'<div class="button submit"><span>'+modules.Lang.chat.delete+'</span></div>'+
										'</div>'+
									'</div>'
								);
							break;
								...
								JSON.parse(e.data).forEach((message) => {
									...
									try {
										switch (message.a) {
											case 'chat':
												let msg = message.p;
												msg.type = /\/(me|em) /i.test(msg.message) ? 'emote': 'message';
												msg.message = msg.message.replace(/\/(me|em) /i, '');
												msg.timestamp = modules.utils.getSimpleTimestamp();
											...
											case 'modMute':
												if (message.p.d === 'o' && API.hasPermission(null, API.ROLE.BOUNCER) && settings.unmuteNotification) {
													pi._tool.log(
														pi._tool.replaceString(lang.notifications.userUnmuted, {user: message.p.t, mod: message.p.m}),
														'chat info'
													);
												}
											break;
											...
											case 'modStaff':
												message.p.u.forEach((e,i,a) => {
													...
													if (e.i === self.id) {
														...
														if (e.p <= 1 && self.role > e.p) window.modToolbar.kill();
														// From Grey or rDJ to anything above
														else if (e.p >= 2 && self.role < e.p) window.modToolbar.init(true);
			...
			afk: function(msg) {
				...
				if (settings.afk) {
					...
					$("#chat-input-field")[0].placeholder = window.originalPlaceholder;
			...
			eta: function(log) {
				...
				if (API.getUser().id == API.getDJ().id) {
					...
					eta.timeLeft = API.getTimeRemaining();
					if (eta.timeLeft >= 3600) {
						eta.hour = Math.floor(eta.timeLeft/60);
						eta.mins = eta.timeLeft%60;
						eta.text = eta.hour+'h'+eta.mins+'m';
					} else if (eta.timeLeft >= 60) {
						eta.mins = Math.floor(eta.timeLeft/60);
						eta.secs = eta.timeLeft%60;
						eta.secs = (eta.secs < 10 ? '0'+eta.secs : eta.secs);
						eta.text = eta.mins+'m'+eta.secs+'s';
					} else {
						eta.text = eta.timeLeft+'s';
					}
				} else {
					eta.estimate = (API.getWaitListPosition())*4*60;
					eta.estimate += API.getTimeRemaining();
					if (eta.estimate >= 3600) {
						eta.hour = Math.floor(eta.estimate/60/60);
						eta.mins = eta.estimate%60;
						eta.text = eta.hour+'h'+eta.mins+'m';
					} else {
			...
			menu: function(choice) {
				switch(choice) {
					...
					case 'languageFlags':
						...
						if (settings.languageFlags) $('.un ~ .pi-languageFlags').show();
						else $('.un ~ .pi-languageFlags').hide();
					break;
					case 'friendsIcons':
						settings.friendsIcons = !settings.friendsIcons;
						pi.dom.friendsIcons.className = settings.friendsIcons ? 'pi-on' : 'pi-off';
						if (settings.friendsIcons) $('.un ~ .pi-friendsIcons').show();
						else $('.un ~ .pi-friendsIcons').hide();
					break;
					case 'bg':
					...
					break;
					case 'gRoleCmdDeletion':
						settings.gRoleCmdSelfDel = !settings.gRoleCmdSelfDel;
						pi.dom.gRoleCmdDeletion.className = settings.gRoleCmdDeletion ? 'pi-on' : 'pi-off';
					break;
					case 'unmuteNotification':
						settings.unmuteNotification = !settings.unmuteNotification;
						pi.dom.unmuteNotification.className = settings.unmuteNotification ? 'pi-on' : 'pi-off';
					break;
					case 'userInfo':
						settings.userInfo = !settings.userInfo;
						pi.dom.userInfo.className = settings.userInfo ? 'pi-on' : 'pi-off';
						if (settings.userInfo) $('.un ~ .userInfo').show();
						else $('.un ~ .userInfo').hide();
					break;
					case 'stuckSkip':
					...
					break;
					case 'lowLevelJoin':
						settings.lowLevelJoin = !settings.lowLevelJoin;
						pi.dom.lowLevelJoin.className = settings.lowLevelJoin ? 'pi-on' : 'pi-off';
					break;
					case 'lowLevelLeave':
						settings.lowLevelLeave = !settings.lowLevelLeave;
						pi.dom.lowLevelLeave.className = settings.lowLevelLeave ? 'pi-on' : 'pi-off';
					break;
					case 'userWoot':
					...
					case 'init':
						...
						pi.dom.languageFlags.className = settings.languageFlags ? 'pi-on' : 'pi-off';
						pi.dom.friendsIcons.className = settings.friendsIcons ? 'pi-on' : 'pi-off';
						if (settings.customBGURL !== null) pi.changeBG(false, settings.customBGURL);
						else void 0;
						pi.dom.oldChat.className = settings.oldChat ? 'pi-on' : 'pi-off';
						pi.toggleStyle('oldChat');
						pi.dom.oldFooter.className = settings.oldFooter ? 'pi-on' : 'pi-off';
						...
						pi.dom.showDeletedMsg.className = settings.showDeletedMsg ? 'pi-on' : 'pi-off';
						pi.dom.confirmDelete.className = settings.confirmDelete ? 'pi-on' : 'pi-off';
						pi.dom.gRoleCmdDeletion.className = settings.gRoleCmdSelfDel ? 'pi-on' : 'pi-off';
						pi.dom.unmuteNotification.className = settings.unmuteNotification ? 'pi-on' : 'pi-off';
						pi.dom.userInfo.className = settings.userInfo ? 'pi-on' : 'pi-off';
						pi.dom.stuckSkip.className = settings.stuckSkip ? 'pi-on' : 'pi-off';
						pi.dom.lengthA.className = settings.songLimit ? 'pi-on' : 'pi-off';
